;(function (f, g) {
	...
	var nt = (f, g, R) =>
		g in f
			...
			: (f[g] = R)
	var a = (f, g, R) => (nt(f, typeof g != 'symbol' ? g + '' : g, R), R)
	const g = {
			'!': 'BANG',
			'&': 'AND',
			'(': 'LEFT_PARENT',
	...
	class U {
		constructor(e) {
			...
			a(this, 'expression')
			e
				? (this.keywordTokens = new Set([...R, ...e]))
				: (this.keywordTokens = R)
		}
		init(e) {
			;(this.currentLine = 0),
...
`,
				...
				return (
					(this.i = t === -1 ? this.expression.length : t),
					this.currentLine++,
					(this.lastColumns = this.i + 1),
					(this.currentColumn = 0),
					this.next()
			...
			let e = g[this.expression[this.i]]
			if (e)
				return new k(
					e,
					this.expression[this.i++],
			...
			if (
				...
				for (
					...
					(this.isLetter(this.expression[t]) ||
						this.isNumber(this.expression[t]) ||
						this.expression[t] === '_' ||
						this.expression[t] === '.');
				...
				for (
					;
					t < this.expression.length && this.expression[t] !== "'";
			...
			return this.hasNext()
				? (this.i++, this.next())
				: new k('EOF', '', this.currentColumn, this.currentLine)
		}
		hasNext() {
	...
	const ce = (r, e, t) =>
		...
		xe = (r) => 3 * r ** 2 - 2 * r ** 3,
		Ee = (r, e, t) => (t < 0 ? (t = 0) : t > 1 && (t = 1), r + (e - r) * t),
		ge = (r, e, t) => {
		...
		me = (r, e) => r % e,
		q = (r, e) => r + Math.random() * (e - r),
		Q = (r, e) => Math.round(r + Math.random() * (e - r)),
		we = (r) => (
			(r = r % 360), (r = (r + 360) % 360), r > 179 && (r -= 360), r
		),
		Se = (r) => {
			...
			return {
				...
				'math.die_roll': pe,
				'math.die_roll_integer': fe,
				'math.exp': Math.exp,
				'math.floor': Math.floor,
				'math.hermite_blend': xe,
				'math.lerp': Ee,
				'math.lerp_rotate': ge,
				'math.ln': Math.log,
				'math.max': Math.max,
		...
		ve = {
			'query.in_range': (r, e, t) =>
				typeof r != 'number' ||
				typeof e != 'number' ||
				typeof t != 'number'
					? (console.error(
							'"query.in_range": value, min and max must be numbers'
					  ),
					  !1)
					: r >= e && r <= t,
			'query.all': (r, ...e) => e.every((t) => t === r),
			'query.any': (r, ...e) => e.some((t) => t === r),
			'query.count': (r) => (Array.isArray(r) ? r.length : 1),
		},
		Y = (r) => ({ ...Se(r), ...ve })
	class F {
		constructor(e, t) {
		...
		updateConfig({
			...
			t !== void 0 && (this.config.convertUndefined = t),
				typeof e == 'function' && (this.config.variableHandler = e),
				!!this.config.useRadians != !!s &&
					(this.env = Object.assign(this.env, Y(!!s)))
		}
		get() {
			return this.env
		}
		flattenEnv(e, t = '', s = {}) {
			...
			for (let i in e) {
				...
				;(n = e[i]) != null && n.__isContext
					? (s[`${t}${o}`] = e[i].env)
					: typeof e[i] == 'object' && !Array.isArray(e[i])
					? this.flattenEnv(e[i], `${t}${i}.`, s)
					: (s[`${t}${o}`] = e[i])
			}
			return s
		}
		...
		getFrom(e) {
			...
			const t =
				(i = this.env[e]) != null
					? i
					: (n = (s = this.config).variableHandler) == null
					? void 0
					: n.call(s, e, this.env)
	...
	class l {
		...
		iterate(e, t) {
			...
			for (let n = 0; n < this.allExpressions.length; n++) {
				...
				;(o !== i && t.has(o)) ||
					(t.add(o), this.setExpressionAt(n, o), o.iterate(e, t))
			}
		}
		some(e) {
			return this.allExpressions.some((t) => e(t) || t.some(e))
		}
	}
	class C extends l {
		constructor() {
	...
	class Re {
		constructor(e) {
			a(this, 'prefixParselets', new Map())
			a(this, 'infixParselets', new Map())
			a(this, 'readTokens', [])
			a(this, 'tokenIterator', new U())
			a(this, 'executionEnv')
			this.config = e
		}
		updateConfig(e) {
		...
		parseInfixExpression(e, t = 0) {
			...
			for (; this.getPrecedence() > t; ) {
				...
				if (!i) throw new Error(`Unknown infix parselet: "${n}"`)
				e = i.parse(this, e, s)
			}
			return e
		}
		...
		match(e, t = !0) {
			return this.lookAhead(0).getType() !== e
				? !1
				: (t && this.consume(), !0)
		}
		lookAhead(e) {
	...
	const O = (r, e) => {
		...
		ye = (r, e) => {
			if (r.setPointer) return r.setPointer(e.eval()), 0
	...
	class M {
		...
		parse(e, t, s) {
			const n = e.parseExpression(this.precedence),
				i = s.getText()
			switch (i) {
				case '+':
					return new h(t, n, i, O)
				case '-':
					return new h(t, n, i, I)
				case '*':
					return new h(t, n, i, b)
				case '/':
					return new h(t, n, i, G)
				case '=':
					return new h(t, n, '=', ye)
				default:
	...
	var u = ((r) => (
		(r[(r.SCOPE = 1)] = 'SCOPE'),
		(r[(r.STATEMENT = 2)] = 'STATEMENT'),
		(r[(r.ASSIGNMENT = 3)] = 'ASSIGNMENT'),
		(r[(r.CONDITIONAL = 4)] = 'CONDITIONAL'),
		(r[(r.ARRAY_ACCESS = 5)] = 'ARRAY_ACCESS'),
		(r[(r.NULLISH_COALESCING = 6)] = 'NULLISH_COALESCING'),
		(r[(r.AND = 7)] = 'AND'),
		(r[(r.OR = 8)] = 'OR'),
		(r[(r.EQUALS_COMPARE = 9)] = 'EQUALS_COMPARE'),
		(r[(r.COMPARE = 10)] = 'COMPARE'),
		(r[(r.SUM = 11)] = 'SUM'),
		(r[(r.PRODUCT = 12)] = 'PRODUCT'),
		(r[(r.EXPONENT = 13)] = 'EXPONENT'),
		(r[(r.PREFIX = 14)] = 'PREFIX'),
		(r[(r.POSTFIX = 15)] = 'POSTFIX'),
		(r[(r.FUNCTION = 16)] = 'FUNCTION'),
		r
	))(u || {})
	class B extends l {
		constructor(t, s) {
		...
		eval() {
			...
			if (typeof t != 'number')
				throw new Error(
					`Cannot use "${
						this.tokenType
					}" operator in front of ${typeof t} "${t}"`
				)
	...
	class D {
		...
		parse(e, t) {
			return new B(t.getType(), e.parseExpression(this.precedence))
		}
	}
	class z extends l {
		constructor(t) {
		...
		eval() {
			...
			return typeof t != 'object'
				...
				: (this.rightExpr.setExecutionEnv(
						new F(t, this.rightExpr.executionEnv.config)
				  ),
				  this.rightExpr.eval())
		}
		toString() {
	...
	class Ce {
		...
		parse(e, t) {
			const s = new y(e.executionEnv, t.getText()),
	...
	class d extends l {
		...
		toString() {
			return `${
				this.brackets[0]
			}${this.expression.toString()}${this.brackets[1]}`
		}
	}
	class be {
		constructor(e = 0) {
	...
	class Ne {
		...
		parse(e, t) {
			...
			return new T(e.match('SEMICOLON', !1) ? s : new z(0))
		}
	}
	class v extends l {
		constructor(t, s = !1) {
	...
	class m extends l {
		...
		get isReturn() {
			if (this.didReturn !== void 0) return this.didReturn
			let t = 0
			for (; t < this.expressions.length; ) {
				const s = this.expressions[t]
				if (s.isBreak || s.isContinue) return !1
				if (s.isReturn) return (this.didReturn = !0), !0
				t++
			}
		...
		get isContinue() {
			return this.wasLoopContinued
				? ((this.wasLoopContinued = !1), !0)
				: !1
		}
		isStatic() {
			let t = 0
			for (; t < this.expressions.length; ) {
				if (!this.expressions[t].isStatic()) return !1
				t++
			}
			return !0
		}
		eval() {
			;(this.didReturn = !1),
				(this.wasLoopBroken = !1),
				(this.wasLoopContinued = !1)
			let t = 0
			for (; t < this.expressions.length; ) {
		...
		toString() {
			...
			for (const s of this.expressions) {
				if (s instanceof C || (s instanceof v && !s.isReturn)) continue
				const n = s.toString()
				n && (t += `${n};`)
			}
			return t
	...
	class ke {
		...
		findReEntryPoint(e) {
			let t = 1,
				s = e.lookAhead(0).getType()
			for (
				;
				s !== 'EOF' &&
				(s == 'CURLY_RIGHT' ? t-- : s === 'CURLY_LEFT' && t++, t !== 0);
		...
		parse(e, t, s) {
			if (
				e.config.useOptimizer &&
				(t.isStatic() && (t = new v(t.eval(), t.isReturn)),
				e.config.earlyReturnsSkipParsing && t.isReturn)
			)
				return (
					e.config.earlyReturnsSkipTokenization ||
						this.findReEntryPoint(e),
					new m([t])
				)
			const n = [t]
			if (!e.match('CURLY_RIGHT', !1))
				do {
					...
					if (e.config.useOptimizer) {
						if (o.isStatic()) {
							if (
								e.config.useAggressiveStaticOptimizer &&
								!o.isReturn
							)
								continue
							o = new v(o.eval(), o.isReturn)
						}
						if (
							e.config.earlyReturnsSkipParsing &&
							(o.isBreak || o.isContinue || o.isReturn)
						)
							return (
								n.push(o),
								e.config.earlyReturnsSkipTokenization ||
					...
					e.match('SEMICOLON') &&
					!e.match('EOF') &&
					!e.match('CURLY_RIGHT', !1)
				)
			return e.match('SEMICOLON'), new m(n)
		}
	}
	class $ extends l {
		constructor(t) {
	...
	class j extends l {
		...
		eval() {
			const t = []
			let s = 0
			for (; s < this.args.length; ) t.push(this.args[s++].eval())
			const n = this.name.eval()
			if (typeof n != 'function')
				throw new Error(`${this.name.toString()} is not callable!`)
			return n(...t)
		}
		toString() {
			let t = `${this.name.toString()}(`
			for (let s = 0; s < this.args.length; s++)
				t += `${this.args[s].toString()}${
					s + 1 < this.args.length ? ',' : ''
				}`
			return `${t})`
		}
	}
	...
	class Me {
		...
		parse(e, t, s) {
			...
			if (!t.setPointer) throw new Error(`"${t.type}" is not an array`)
	...
	class X extends l {
		...
		eval() {
			...
			for (; s < t; ) {
				...
				if (this.expression.isContinue) continue
				if (this.expression.isReturn) return n
			}
			return 0
		}
		toString() {
			return this.isNoopLoop
				? ''
				: `loop(${this.count.toString()},${this.expression.toString()})`
		}
	}
	class Pe {
		constructor(e = 0) {
	...
	class W extends l {
		constructor(t, s, n) {
			...
			if (
				((this.variable = t),
				(this.arrayExpression = s),
				(this.expression = n),
				!this.variable.setPointer)
			)
				throw new Error(
		...
		eval() {
			...
			let s = 0
			for (; s < t.length; ) {
				;(i = (n = this.variable).setPointer) == null ||
					i.call(n, t[s++])
				const o = this.expression.eval()
				if (this.expression.isBreak) break
				if (this.expression.isContinue) continue
				if (this.expression.isReturn) return o
			}
			return 0
		}
		toString() {
	...
	class H extends l {
		constructor(t, s, n) {
			...
			a(this, 'leftResult')
			;(this.leftExpression = t),
				(this.thenExpression = s),
		...
		setExpressionAt(t, s) {
			t === 0
				? (this.leftExpression = s)
				: t === 1
				? (this.thenExpression = s)
				: t === 2 && (this.elseExpression = s)
		}
		get isReturn() {
			return this.leftResult === void 0
		...
		get isBreak() {
			return this.leftResult === void 0
				? this.thenExpression.isBreak && this.elseExpression.isBreak
				: this.leftResult
		...
		toString() {
			return this.elseExpression instanceof C
				? `${this.leftExpression.toString()}?${this.thenExpression.toString()}`
				: `${this.leftExpression.toString()}?${this.thenExpression.toString()}:${this.elseExpression.toString()}`
		}
	}
	class Qe {
		constructor(e = 0) {
		...
		parse(e, t, s) {
			let n = e.parseExpression(this.precedence - 1),
				i
			return (
				e.match('COLON')
					? (i = e.parseExpression(this.precedence - 1))
					: (i = new C()),
				e.config.useOptimizer && t.isStatic()
					? t.eval()
						? n
						: i
					: new H(t, n, i)
	...
	const Ye = new Qe(u.CONDITIONAL)
	class De {
		constructor(e = 0) {
			this.precedence = e
		}
		parse(e, t, s) {
			return e.match('QUESTION')
				? new h(t, e.parseExpression(this.precedence), '??', (n, i) => {
						var o
	...
	class te extends Re {
		constructor(e) {
			super(e),
				this.registerPrefix('NAME', new Ce()),
				this.registerPrefix('STRING', new Oe()),
				this.registerPrefix('NUMBER', new Te()),
				this.registerPrefix('TRUE', new ee(u.PREFIX)),
				this.registerPrefix('FALSE', new ee(u.PREFIX)),
				this.registerPrefix('RETURN', new Ne()),
				this.registerPrefix('CONTINUE', new _e()),
				this.registerPrefix('BREAK', new Ue()),
				this.registerPrefix('LOOP', new Pe()),
				this.registerPrefix('FOR_EACH', new $e()),
				this.registerInfix('QUESTION', new De(u.CONDITIONAL)),
				this.registerPrefix('LEFT_PARENT', new be()),
				this.registerInfix('LEFT_PARENT', new Ie(u.FUNCTION)),
				this.registerInfix('ARRAY_LEFT', new Me(u.ARRAY_ACCESS)),
				this.registerPrefix('CURLY_LEFT', new Le(u.SCOPE)),
				this.registerInfix('SEMICOLON', new ke(u.STATEMENT)),
				this.registerPrefix('MINUS', new D(u.PREFIX)),
				this.registerPrefix('BANG', new D(u.PREFIX)),
				this.registerInfix('PLUS', new M(u.SUM)),
				this.registerInfix('MINUS', new M(u.SUM)),
				this.registerInfix('ASTERISK', new M(u.PRODUCT)),
				this.registerInfix('SLASH', new M(u.PRODUCT)),
				this.registerInfix('EQUALS', new Fe(u.EQUALS_COMPARE)),
				this.registerInfix('BANG', new Ge(u.EQUALS_COMPARE)),
				this.registerInfix('GREATER', new qe(u.COMPARE)),
				this.registerInfix('SMALLER', new He(u.COMPARE)),
				this.registerInfix('AND', new Be(u.AND)),
				this.registerInfix('OR', new ze(u.OR)),
				this.registerInfix('ASSIGN', new M(u.ASSIGNMENT))
		}
	}
	class je {
		constructor(e = 0) {
		...
		parse(e, t) {
			...
			do {
				...
				if (o instanceof $) i ? s.push(o.eval()) : (i = o.eval())
				else if (o instanceof m || o instanceof d) n = o
				else
					throw new Error(
			...
			return new Ve(e.functions, i, s, n)
		}
	}
	class Ve extends l {
		constructor(t, s, n, i) {
		...
		Object.defineProperty(
			{
				...
				NameExpression: y,
				NumberExpression: z,
				PostfixExpression: Xe,
				PrefixExpression: B,
				ReturnExpression: T,
				StatementExpression: m,
				StaticExpression: v,
				StringExpression: $,
				TernaryExpression: H,
				VoidExpression: C,
	...
	class se {
		constructor(e = {}, t = {}) {
			...
			a(this, 'parser')
			;(this.config = t),
				t.useOptimizer === void 0 && (this.config.useOptimizer = !0),
				t.useCache === void 0 && (this.config.useCache = !0),
				t.earlyReturnsSkipParsing === void 0 &&
					(this.config.earlyReturnsSkipParsing = !0),
				t.earlyReturnsSkipTokenization === void 0 &&
					(this.config.earlyReturnsSkipTokenization = !0),
				t.convertUndefined === void 0 &&
					(this.config.convertUndefined = !1),
				(this.parser = new te({ ...this.config, tokenizer: void 0 })),
				this.updateExecutionEnv(e, t.assumeFlatEnvironment)
		}
		updateConfig(e) {
			;(e = Object.assign(this.config, e)),
				e.tokenizer && this.parser.setTokenizer(e.tokenizer),
				this.parser.updateConfig({ ...this.config, tokenizer: void 0 }),
				this.executionEnvironment.updateConfig(e)
		}
		updateExecutionEnv(e, t = !1) {
			;(this.executionEnvironment = new F(e, {
		...
		parse(e) {
			...
			return (
				((n = this.config.useOptimizer) != null ? n : !0) &&
					t.isStatic() &&
					(t = new v(t.eval())),
				((i = this.config.useCache) == null || i) &&
					(this.totalCacheEntries >
						(this.config.maxCacheSize || 256) && this.clearCache(),
					(this.expressionCache[e] = t),
					this.totalCacheEntries++),
				t
			)
		}
		rearrangeOptimally(e) {
			...
			do
				(t = e.toString()),
					(e = e.walk((s) => {
						if (s instanceof h) {
							let n = s.allExpressions[0],
								i = s.allExpressions[1]
							if (n instanceof h && i.isStatic()) {
								let o = n.allExpressions[1],
									p = n.allExpressions[0]
								if (
									!p.isStatic() &&
									!(p instanceof h) &&
									o.isStatic()
		...
		resolveStatic(e) {
			return (
				(e = this.rearrangeOptimally(e)),
				(e = e.walk((t) => {
					if (!(t instanceof $) && t.isStatic())
						return new v(t.eval())
				})),
				(e = e.walk((t) => {
					if (t instanceof h) {
						switch (t.operator) {
							...
							case '*': {
								...
								if (n)
									return t.allExpressions.find((o) => o !== n)
							}
							case '/': {
								const s = t.allExpressions[0],
									n = t.allExpressions[1]
								if (n.isStatic() && n.eval() === 1) return s
			...
			return (
				(e = e.walk((n) => {
					if (n instanceof y) {
						const i = n.toString()
						if (
							(!i.startsWith('v.') && !i.startsWith('t.')) ||
							i.length === 3
						)
							return
						const o = i.startsWith('v.') ? 'v.' : 't.'
						if (s.has(i)) n.setName(s.get(i))
						else {
							const p = `${o}v${s.size}`
	...
	function Ke(r) {
		if (r instanceof T) return new d(r.allExpressions[0], '()')
		if (!(r instanceof m) || r.allExpressions.length > 1) return r
		const e = r.allExpressions[0]
		return e instanceof T ? new d(e.allExpressions[0], '()') : r
	}
	class Ze extends te {
		constructor(t) {
			super(t)
			a(this, 'functions', new Map())
			a(this, 'classes', new Map())
			this.registerPrefix('FUNCTION', new je())
		}
		reset() {
	...
	class Je {
		constructor(e) {
			...
			;(this.parser = new Ze({
				useCache: !1,
				useOptimizer: !0,
				useAggressiveStaticOptimizer: !0,
				keepGroups: !0,
				earlyReturnsSkipParsing: !1,
				earlyReturnsSkipTokenization: !1,
			})),
				this.parser.setExecutionEnvironment(new F(this.parser, e)),
				this.parser.setTokenizer(new U(new Set(['function'])))
		}
		get functions() {
		...
		transform(e) {
			...
			n = n.walk((c) => {
				...
				if (
					...
					(!(
						(oe = (ie = c.name.name).startsWith) != null &&
						oe.call(ie, 'f.')
					) &&
						!(
							(he = (ae = c.name.name).startsWith) != null &&
							he.call(ae, 'function.')
						))
				)
					return
				const tt = c.name.name.replace(/(f|function)\./g, ''),
					st = c.args
				let [re, L] = (ue = this.functions.get(tt)) != null ? ue : []
				if (!L || !re) return
				L = L.replace(/(a|arg)\.(\w+)/g, (E, S, x) => {
					var N, P
					return (
						(P =
							(N = st[re.indexOf(x)]) == null
								? void 0
								: N.toString()) != null
							? P
							: '0'
					).replace(/(t|temp)\./, 'outer_temp.')
				})
				let A = Ke(t.parse(L))
				if (A instanceof m) {
					const E = A.allExpressions.some((x) => x instanceof T),
						S = E || A.some((x) => x instanceof T)
					;(A = t.parse(
						`({${L}}+${
							S
								? E
				...
				return (
					(A = A.walk((E) => {
						if (E instanceof y) {
							...
							const w = x.shift(),
								[N, ...P] = x,
								le = P.length > 0 ? '.' + P.join('.') : ''
							if (w === 't' || w === 'temp') {
								let _ = ne.get(N)
								_ || ((_ = `t.__scvar${s++}`), ne.set(N, _)),
									E.setName(`${_}${le}`)
							} else
								w === 'outer_temp' && E.setName(`t.${N}${le}`)
							return
							...
							const S = new y(
								...
								x = E.allExpressions[0]
							return new h(S, x, '=', () => {
								S.setPointer(x.eval())
							})
						} else if (E instanceof m) {
							const S = []
							for (let x = 0; x < E.allExpressions.length; x++) {
								const w = E.allExpressions[x]
								if (w instanceof H && w.hasReturn) {
									et(w, E.allExpressions.slice(x + 1)),
										S.push(w)
									break
								} else if (w.isReturn) {
									S.push(w)
			...
			return (
				t.resolveStatic(p),
				!i && o ? `return ${p.toString()};` : p.toString()
			)
		}
		reset() {
			this.functions.clear()
	...
	function et(r, e) {
		...
		const t = r.allExpressions[2].isReturn ? 1 : 2,
			s = r.allExpressions[t]
		s instanceof C ||
			(s instanceof d && s.allExpressions[0] instanceof m
				? e.unshift(...s.allExpressions)
				: e.unshift(s)),
			e.length > 0 && r.setExpressionAt(t, new d(new m(e), '{}'))
	}
	;(f.Context = de),
		(f.CustomMolang = Je),
		(f.Molang = se),
